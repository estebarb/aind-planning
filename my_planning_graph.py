...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (
            bool(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)) or
            bool(set(node_a2.action.effect_add) & set(node_a1.action.effect_rem))
        )
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (
            ...
            bool(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)) or
            bool(set(node_a2.action.effect_add) & set(node_a1.action.precond_neg)) or
            bool(set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos))
        )
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return any([p1.is_mutex(p2) for p1 in node_a1.parents for p2 in node_a2.parents])
    ...
    def h_levelsum(self) -> int:
        ...
        def level_sum_per_goal(goal):
            for level, states in enumerate(self.s_levels):
                if goal in states:
                    return level
            return float('Inf')

        return sum([level_sum_per_goal(PgNode_s(g, True)) for g in self.problem.goal])
